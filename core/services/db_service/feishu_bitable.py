...
class FeishuBitableManager:
        ...
        try:
            ...
            if resp.status_code != 200:
                ...
                if data.get("code") == 1254045:
                    if "异常日志" in fields:
                        logger.warning("飞书多维表格缺少 [异常日志] 字段，忽略该字段重试。")
                        fields.pop("异常日志", None)
                        return self.update_record(record_id, fields)
                    elif "视频文件" in fields:
                        logger.warning("飞书多维表格缺少 [视频文件] 或对应附件字段，忽略该字段重试。视频可能无法上传到飞书，但由于保留了状态流转，系统能继续运行。建议在多维表格中添加该【附件】类型字段！")
                        fields.pop("视频文件", None)
                        return self.update_record(record_id, fields)
            resp.raise_for_status()
            data = resp.json()
            if data.get("code") == 0:
                ...
                if data.get("code") == 1254045:
                    if "异常日志" in fields:
                        ...
                        logger.warning("飞书多维表格缺少 [视频文件] 或对应附件字段，忽略该字段重试。建议在多维表格中添加该【附件】类型字段！")
                        fields.pop("视频文件", None)
                        return self.update_record(record_id, fields)
    ...
    def upload_attachment_and_update_record(self, record_id: str, file_path: str):
        # 根据用户需求，不再直接上传物理视频附件至飞书，而是存文件名文本以记录生成完成。
        import os
